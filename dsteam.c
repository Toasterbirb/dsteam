...
static int bh, mw, mh;
static int dmx = 0; /* put dsteam at this x offset */
static int dmy = 0; /* put dsteam at this y offset (measured from the bottom if topbar is 0) */
static unsigned int dmw = 0; /* make dsteam this wide */
...
insert:
	...
	case XK_Return:
	case XK_KP_Enter:
		/* Option selected, launch the game */
...
readdb(void)
{
	...
	char* configpath = "/.config/dsteam/db";
	char dbpath[512];
	strcat(dbpath, homepath);
	strcat(dbpath, configpath);

	const char* readmode = "r";
	FILE *fp;
	fp = fopen(dbpath, readmode);
...
main(int argc, char *argv[])
{
	...
	// Enable case-insensitivity if toggled on in the config file
	if (caseinsensitive)
	{
		fstrncmp = strncasecmp;
		fstrstr = cistrstr;
